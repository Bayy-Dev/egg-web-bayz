...
case "${FW}" in
    ...
    laravel)
        if ! which php >/dev/null 2>&1; then
            echo "PHP tidak ada, installing..."
            apt-get update -qq
            apt-get install -y -qq php-cli php-curl php-mbstring php-xml php-zip php-mysql 2>/dev/null || true
        fi
        which php || { echo "ERROR: PHP gagal diinstall!"; exit 1; }
    ...
    php)
        if ! which php >/dev/null 2>&1; then
            echo "PHP tidak ada, installing..."
            apt-get update -qq
            apt-get install -y -qq php-cli php-curl php-mbstring php-xml php-zip php-mysql 2>/dev/null || \
            apt-get install -y -qq php 2>/dev/null || true
        fi
        which php || { echo "ERROR: PHP gagal diinstall!"; exit 1; }
    ...
    *)
        ...
        echo "Pilihan: auto nextjs nuxt react-vite vue react-cra express node laravel php static"
        exit 1
        ;;
esac
